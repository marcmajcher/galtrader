   ...
   `20
    ...
    if (cic=1 and mid$(str_val$,cic,1)="-" and len(str_val$)>1) then
      iterate
        ...
        if ascii(mid$(str_val$,cic,1))<48 or ascii(mid$(str_val$,cic,1))>57
 then
      integerp=FALSE
        end if
      next cic
...
!      if a=val(a$) then
    ...
    for i=1 to ntcargo
      ...
      if ((4*c(i)::tprice)`5E2+17*c(i)::ttech`5E3) <> checksum then
        goto 10000
      end if
    next i
...
tune_comlink:
     ...
     a$="Channel 1 setting (01 to 99) `5B"+str$(pr::chan1)+"`5D> "
     call display(1,a$)
     a = val(a$)
...
tune_second:
     a$="Channel 2 setting (01 to 99) `5B"+str$(pr::chan2)+"`5D> "
     call display(1,a$)
     a = val(a$)
     if a<0 or a>99 or a<>int(a) then
       call display(33,"Invalid Channel Number")
...
1400 ! ---------------------------------------------------------------
     ...
     ! returns with the action file locked (if rec found) An update on it sh
ould
...
2030 for i=1 to numplanets
        ...
        if sqr(pt::xp`5E2+pt::yp`5E2)<5 then pt::zone=5
        end if
...
2300 ! ------------------- Interplanetary Jump ----------------------
     ...
     for i=1 to numplanets
       ra=atan(xp(i),yp(i))
       r=sqr(xp(i)`5E2+yp(i)`5E2)
       d=sqr((xp(i)-pt::xp)`5E2+(yp(i)-pt::yp)`5E2)
...
2320 while 1=1
     ...
     next
2330 newplanet=i\d=sqr((xp(newplanet)-pt::xp)`5E2+(yp(newplanet)-pt::yp)`5E2)
    ...
    and melt<>3 and super_user_mode%=0) then
     ...
     ring=sqr(xp(pr::planet)`5E2+yp(pr::planet)`5E2)
     get #1%, key #0% eq name$(pr::planet), wait 60\free #1%
...
add_to_action:
     ...
     if (pt::zone=2 and rnd>.5) or (pt::zone=3 and rnd>.2) or (pt::zone=4) t
hen
       goto 2381
     end if
...
2590 if a=12 then pr::maxmissile=pr::maxmissile+3 end if
     ...
     if a=2 or a=4 or a=6 or a=10 then
         b=int(pr::equip(2)+4*pr::equip(4)+8*pr::equip(6)+pr::equip(10)/2)*3
00
...
2691 a$="Item to sell> "\call display(1,a$)\a=val(a$)
     if a<0 or a>10 or a<>int(a) then goto 2691 else if a=0 then return end
 if
...
launch_ebomb:
     for ii=1 to noship
      if t(ii)::ship=-1 or t(ii)::ship=20 or t(ii)::ship=17 then iterate end
 if
